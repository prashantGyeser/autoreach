class Keywords
  ...
  def extract(text)
    ...
    highscore_keyword_extract(text)
  end
  ...
  def highscore_keyword_extract(text)
    keyword_text = Highscore::Content.new text, blacklist
    keyword_text.configure do
      set :multiplier, 2
      set :upper_case, 3
      set :long_words, 2
  ...
  def blacklist
    Highscore::Blacklist.load stopwords_list
  end
